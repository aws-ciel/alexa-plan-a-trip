...
const TripIntnetHandler = {
    canHandle(handlerInput) {
        return handlerInput.requestEnvelope.request.type === 'IntentRequest'
            && handlerInput.requestEnvelope.request.intent.name === 'TripIntent';
    },
    handle(handlerInput) {
        // 初始化所有地點的得分 
...
const YesNoIntentHandler = {
    ...
    handle(handlerInput) {
        ...
        if (attrs.questionCount >= MAX_QUESTION_COUNT) {
            ...
            //將結果回傳
            let speechText = `Based on my calculation. You'll enjoy ${topDestination} for your next vacation. Thanks for playing Plan the trip.`;
            return handlerInput.responseBuilder
                .speak(speechText)
                .getResponse();
...
const LaunchRequestHandler = {
    ...
    handle(handlerInput) {
        const speechText = 'Welcome to Plan the trip. I can find the best vacation destination for you. To start it, just say plan it or start planning.';
        return handlerInput.responseBuilder
            .speak(speechText)
            .reprompt(speechText)
            .getResponse();
